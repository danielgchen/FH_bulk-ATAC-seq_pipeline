...
def detect_adapters():
    ...
    '''
    fastqs = glob('data/raw_data/*R1_001.fastq.gz')  # TODO change this to correct one
    for fastq in fastqs:
        # get data
        raw_fastq1 = fastq
        raw_fastq2 = raw_fastq1.replace('R1_001', 'R2_001')
        adapters = 'qc_reports/adapter_detection/' + raw_fastq1.split('/')[-1].replace('R1_001.fastq.gz', 'adapters.fa')
...
def prep_map_reference(configs):
    ...
    run_command(f'wget {gtf_link} -O data/reference/hg38.refGene.gtf.gz', 'DOWNLOADING REFERENCE GTF')
    ...
    run_command(f'STAR --runThreadN 10 --runMode genomeGenerate --genomeDir data/reference_STAR --genomeFastaFiles {fasta_files} --sjdbGTFfile data/reference/hg38.refGene.gtf --sjdbOverhang {overhang}', 'INDEXING GENOME')
...
def qc_mapped_data():
    ...
    for in_bam in bam_files:
        ...
        run_command(f'java -Xmx10g -jar $EBROOTPICARD/picard.jar MarkDuplicates -I {in_bam} -O {out_bam} -M {metrics}', 'MARKING DUPLICATES')
        run_command(f'samtools index {in_bam}', 'INDEXING BAM')
        run_command(f'samtools idxstats {in_bam} > {stats}', 'COUNTING CHROMOSOMES')
        run_command(f'infer_experiment.py -r data/reference/hg38_RefSeq.bed -i {in_bam} > {strands}', 'INFERRING STRAND')
        run_command(f'read_distribution.py -i {in_bam} -r data/reference/hg38_RefSeq.bed > {distribution}', 'GATHERING READ DISTRIBUTION')
    # randomly gather genes
    lines = []
    with open('data/reference/hg38_RefSeq.bed', 'rt') as f:
        for line in f:
            lines.append(line)
    ...
    run_command(f'geneBody_coverage.py -i {bam_files} -r data/reference/hg38_RefSeq_2k_genes.bed -o qc_reports/mapping_reads/rseqc', 'GATHERING GENE BODY COVERAGE')
...
def clean_up():
    ...
    run_command(f'rm {dup_bams}', 'REMOVING DUP VALUES')
